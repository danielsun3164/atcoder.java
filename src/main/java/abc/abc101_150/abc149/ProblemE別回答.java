 ...
 * 解説通りに作成したソースコード
 *
 * 解法 2(高速フーリエ変換)の実装<br/>
 * https://atcoder.jp/contests/abc149/submissions/23315353 にも参考
 */
public class ProblemE別回答 {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			Arrays.fill(c, 0L);
			IntStream.range(0, n).forEach(i -> c[a[i]]++);
			long[] r = Convolution.convolutionLong(c, c);
			int number = r.length - 1;
			long answer = 0L;
			while (m > 0L) {
				answer += Math.min(m, r[number]) * number;
				m -= Math.min(m, r[number]);
				number--;
			}
