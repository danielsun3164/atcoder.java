 ...
 * 解説通りに実装したソースコード
 *
 * https://atcoder.jp/contests/abc225/editorial/2834 の解説の実装<br/>
 * https://atcoder.jp/contests/abc225/submissions/26879083 にも参考
 */
public class ProblemH {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			int[] a = IntStream.range(0, k).map(i -> scanner.nextInt()).toArray();
			fact = new long[(n << 1) + 1];
			invFact = new long[(n << 1) + 1];
			fact[0] = 1L;
			IntStream.range(1, (n << 1) + 1).forEach(i -> fact[i] = fact[i - 1] * i % MOD);
			invFact[n << 1] = Convolution.powMod(fact[n << 1], MOD - 2, MOD);
			IntStream.range(0, n << 1).map(i -> (n << 1) - 1 - i)
					.forEach(i -> invFact[i] = invFact[i + 1] * (i + 1) % MOD);
			if (0 == k) {
				System.out.println(comb(n + m - 1, (m << 1) - 1) * fact[m] % MOD);
				return;
			}
			Deque<long[]> deque = new ArrayDeque<>();
			IntStream.range(0, k - 1).forEach(i -> deque.addLast(f1(a[i + 1] - a[i] + 1)));
			deque.addLast(f2(a[0]));
			deque.addLast(f2(n + 1 - a[k - 1]));
			while (deque.size() >= 2) {
				long[] a1 = deque.pollFirst(), a2 = deque.pollFirst();
				long[] a3 = Convolution.convolution(a1, a2, MOD);
				if (a3.length > m - k + 1) {
					deque.addLast(Arrays.copyOf(a3, m - k + 1));
				} else {
					deque.addLast(a3);
				}
			}
			System.out.println(deque.peekFirst()[m - k] * fact[m - k] % MOD);
		}
	}
	...
	private static long[] f1(int n) {
		return IntStream.range(0, n - 1).mapToLong(k -> comb(n + k - 1, (k << 1) + 1)).toArray();
	}
	...
	private static long[] f2(int n) {
		return IntStream.range(0, n).mapToLong(k -> comb(n + k - 1, k << 1)).toArray();
	}
	...
	private static long comb(int n, int k) {
		return (0 == n) ? 1L : fact[n] * invFact[n - k] % MOD * invFact[k] % MOD;
	}
