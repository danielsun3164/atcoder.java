 ...
 * 解説通りに実装したソースコード
 *
 * https://atcoder.jp/contests/abc231/editorial/3060 の最小費用の実装<br/>
 * https://atcoder.jp/contests/abc231/submissions/27806120 にも参考
 */
public class ProblemH別回答 {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			int[] hMin = new int[h], wMin = new int[w];
			Arrays.fill(hMin, INF);
			Arrays.fill(wMin, INF);
			Data[] data = IntStream.range(0, n).mapToObj(i -> {
				int a = scanner.nextInt() - 1, b = scanner.nextInt() - 1, c = scanner.nextInt();
				hMin[a] = Math.min(hMin[a], c);
				wMin[b] = Math.min(wMin[b], c);
				return new Data(a, b, c);
			}).toArray(Data[]::new);
			int s = 0, t = h + w + 1;
			MinCostFlowGraph graph = new MinCostFlowGraph(t + 1);
			IntStream.rangeClosed(1, h).forEach(i -> graph.addEdge(s, i, 1L, 0L));
			IntStream.rangeClosed(1, w).forEach(i -> graph.addEdge(h + i, t, 1L, 0L));
			Arrays.stream(data).filter(di -> di.c < hMin[di.a] + wMin[di.b])
					.forEach(di -> graph.addEdge(di.a + 1, di.b + h + 1, 1L, di.c - hMin[di.a] - wMin[di.b] + INF));
			graph.addEdge(s, t, Math.min(h, w), INF);
			long answerBase = Arrays.stream(hMin).asLongStream().sum() + Arrays.stream(wMin).asLongStream().sum();
			Result result = graph.flow(s, t, Math.min(h, w));
			System.out.println(answerBase + result.cost - INF * result.cap);
		}
	}
