...
public class ProblemE {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			IntStream.range(0, m).forEach(i -> {
				...
				datas.add(new Data(a, b, c, -1));
			});
			IntStream.range(0, q).forEach(i -> {
				int u = scanner.nextInt() - 1, v = scanner.nextInt() - 1, w = scanner.nextInt();
				datas.add(new Data(u, v, w, i));
			});
			Collections.sort(datas, (x, y) -> Integer.compare(x.w, y.w));
			DisjointSetUnion dsu = new DisjointSetUnion(n);
			datas.forEach(data -> {
				if (!dsu.same(data.u, data.v)) {
					if (-1 != data.index) {
						answers[data.index] = "Yes";
					} else {
						dsu.merge(data.u, data.v);
					}
				} else {
					if (-1 != data.index) {
						answers[data.index] = "No";
					}
				}
			});
	...
	private static class Data {
		...
		Data(int u, int v, int w, int index) {
			this.u = u;
			this.v = v;
			this.w = w;
			this.index = index;
