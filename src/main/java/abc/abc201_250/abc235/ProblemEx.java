 ...
 * 解説通りに実装したソースコード
 *
 * https://atcoder.jp/contests/abc235/editorial/3250 の実装<br/>
 * https://atcoder.jp/contests/abc235/submissions/28592333 にも参考
 */
public class ProblemEx {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			Map<Integer, List<Edge>> map = new TreeMap<>();
			IntStream.range(0, m).forEach(i -> {
				int a = scanner.nextInt() - 1, b = scanner.nextInt() - 1, c = scanner.nextInt();
				if (!map.containsKey(c)) {
					map.put(c, new ArrayList<>());
				}
				map.get(c).add(new Edge(a, b));
			});
			List<List<Integer>> to = new ArrayList<>();
			IntStream.range(0, n).forEach(i -> to.add(new ArrayList<>()));
			int[] top = IntStream.range(0, n).toArray();
			List<Boolean> root = new ArrayList<>();
			IntStream.range(0, n).forEach(i -> root.add(true));
			DisjointSetUnion dsu = new DisjointSetUnion(n);
			for (Entry<Integer, List<Edge>> entry : map.entrySet()) {
				Set<Integer> set = new TreeSet<>();
				for (Edge edge : entry.getValue()) {
					if (!dsu.same(edge.a, edge.b)) {
						set.add(dsu.leader(edge.a));
						set.add(dsu.leader(edge.b));
						dsu.merge(edge.a, edge.b);
					}
				}
				for (int v : set) {
					if (v == dsu.leader(v)) {
						root.set(top[v], false);
						root.add(true);
						to.add(new ArrayList<>(Arrays.asList(top[v])));
						top[v] = to.size() - 1;
					}
				}
				for (int v : set) {
	...
	/**
	 * 配列a x 配列b の最初のk+1項を計算する
	 *
	 * @param a
	 * @param b
	 * @param k
	 * @return 配列a x 配列b の最初のk+1項
	 */
	private static long[] merge(long[] a, long[] b, int k) {
		long[] c = Convolution.convolution(a, b, MOD);
		return (c.length > k + 1) ? Arrays.copyOf(c, k + 1) : c;
	}

	private static long[] dfs(int k, int v, List<List<Integer>> to) {
		long[] dp = new long[] { 1L, 0L };
		for (int u : to.get(v)) {
			dp = merge(dfs(k, u, to), dp, k);
		}
		int c = to.get(v).size();
		if ((0 != c) && (c < dp.length)) {
			dp[c] = (dp[c] + MOD - 1) % MOD;
