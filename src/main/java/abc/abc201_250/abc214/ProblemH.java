...
public class ProblemH {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			IntStream.range(0, p).forEach(i -> scc[i].stream().forEach(u -> belongs[u] = i));
			long[] x = new long[p];
			Arrays.fill(x, 0L);
			IntStream.range(0, n).forEach(i -> x[belongs[i]] += scanner.nextInt());
			long[] accum = new long[p + 1];
			Arrays.fill(accum, 0L);
			IntStream.range(0, p).forEach(i -> accum[i + 1] = accum[i] + x[i]);
			MinCostFlowGraph network = new MinCostFlowGraph(2 * p + 1);
			IntStream.range(0, p).forEach(i -> {
				network.addEdge(2 * i, 2 * i + 1, 1, 0);
				network.addEdge(2 * i, 2 * i + 1, k, x[i]);
				network.addEdge(2 * i + 1, 2 * p, k, accum[p] - accum[i + 1]);
			});
			IntStream.range(0, m).forEach(i -> {
				int u = belongs[a[i]], v = belongs[b[i]];
				if (u != v) {
					network.addEdge(2 * u + 1, 2 * v, k, accum[v] - accum[u + 1]);
				}
			});
			int s = belongs[0];
			System.out.println(((accum[p] - accum[s]) * k - network.flow(2 * s, 2 * p, k).cost));
		}
	}
