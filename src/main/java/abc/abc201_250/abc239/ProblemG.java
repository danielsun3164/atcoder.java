 ...
 * 解説通りに実装したソースコード
 *
 * https://atcoder.jp/contests/abc239/editorial/3393 の実装<br/>
 * https://atcoder.jp/contests/abc239/submissions/29232429 にも参考
 */
public class ProblemG {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			MaxFlowGraph graph = new MaxFlowGraph(n << 1);
			int s = n, t = n - 1;
			IntStream.range(0, n).forEach(i -> graph.addEdge(i, i + n, c[i]));
			IntStream.range(0, m).forEach(i -> {
				graph.addEdge(a[i] + n, b[i], INF);
				graph.addEdge(b[i] + n, a[i], INF);
			});
			System.out.println(graph.flow(s, t));
			boolean[] minCut = graph.minCut(s);
			int[] answer = IntStream.range(0, n).filter(i -> minCut[i] && !minCut[i + n]).toArray();
			System.out.println(answer.length);
			System.out.println(
					Arrays.stream(answer).mapToObj(ai -> String.valueOf(ai + 1)).collect(Collectors.joining(" ")));
		}
	}
