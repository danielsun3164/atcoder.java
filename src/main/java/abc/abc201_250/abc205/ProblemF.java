...
public class ProblemF {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			// src から全ての R_i へ容量 1 の辺を張る。
			IntStream.range(0, h).forEach(i -> g.addEdge(s, i, 1L));
			// 各 i(1≤i≤N) について以下の処理を行う。
			IntStream.range(0, n).forEach(i -> {
				int a = scanner.nextInt() - 1, b = scanner.nextInt() - 1, c = scanner.nextInt(), d = scanner.nextInt();
				int ui = h + w + i, vi = h + w + n + i;
				// R_ai,R_ai+1,…,R_ci から U_i ヘ容量 1 の辺を張る。
				IntStream.range(a, c).forEach(j -> g.addEdge(j, ui, 1L));
				// U_i から V_i へ容量 1 の辺を張る。
				g.addEdge(ui, vi, 1L);
				// V_i から C_bi,C_bi+1,…,C_di へ容量 1 の辺を張る。
				IntStream.range(b, d).forEach(j -> g.addEdge(vi, h + j, 1L));
			});
			// 全ての C_i から dst へ容量 1 の辺を張る。
			IntStream.range(0, w).forEach(i -> g.addEdge(h + i, t, 1L));
			System.out.println(g.flow(s, t));
		}
	}
