 ...
 * 解説通りに実装したソースコード
 *
 * https://atcoder.jp/contests/abc237/editorial/3339 の実装
 */
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			IntStream.range(0, m).forEach(i -> {
				...
				edges[u].add(new Edge(v, Math.max(0L, h[v] - h[u])));
				edges[v].add(new Edge(u, Math.max(0L, h[u] - h[v])));
			});
			...
			System.out.println(IntStream.range(0, n).mapToLong(i -> h[0] - h[i] - dist[i]).max().getAsLong());
		}
	}
