 ...
 * 解説通りに実装したソースコード
 *
 * https://atcoder.jp/contests/abc240/editorial/3428 の実装<br/>
 * https://atcoder.jp/contests/abc240/submissions/33133126 にも参考
 */
public class ProblemEx {
	...
	private static List<int[]> trie = new ArrayList<>();
		...
		try (Scanner scanner = new Scanner(System.in)) {
			...
			for (int i = 1; i <= n; i++) {
				...
				for (int j = 0; j < B; j++) {
					...
					int c = s[i + j] - '0';
					v = trie.get(v)[c];
					list.add(new int[] { -trie.get(v)[2], i, i + j });
				}
			}
			list.sort((x, y) -> Arrays.compare(y, x));
			IntSegTree seg = new IntSegTree(n + 1, (a, b) -> Math.max(a, b), () -> -INF);
			seg.set(0, 0);
			for (int[] a : list) {
				int l = a[1], r = a[2], value = Math.max(seg.prod(0, l) + 1, seg.prod(r, r + 1));
				seg.set(r, value);
			}
			System.out.println(seg.prod(0, n + 1));
		}
	}
	 ...
	 * trieに再帰的にidを付与する
	 *
	 * @param v 現在処理するインデックス
	 */
	private static void dfs(int v) {
		trie.get(v)[2] = id++;
		IntStream.range(0, 2).filter(i -> trie.get(v)[i] != -1).forEach(i -> dfs(trie.get(v)[i]));
	}
