 ...
 * 解説通りに実装したソースコード
 *
 * https://atcoder.jp/contests/abc213/submissions/24839086 と https://atcoder.jp/contests/abc213/submissions/25074532
 * にも参考
 */
public class ProblemH {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			IntStream.range(0, m).forEach(i -> {
				...
				p[a][b] = new int[t + 1];
				p[a][b][0] = 0;
				IntStream.rangeClosed(1, t).forEach(j -> p[a][b][j] = scanner.nextInt());
			});
			int[][] d = new int[n][t + 1];
			d[0][0] = 1;
			onlineConvolution(edges, p, d, 0, t + 1);
			System.out.println(d[0][t]);
		}
	}
	 ...
	 *
	 * @param edges 道の一覧
	 * @param p     道のそれぞれの長さの本数の一覧
	 * @param d     結果
	 * @param l     計算対象範囲の最小値
	 * @param r     計算対象範囲の最大値
	 */
	private static void onlineConvolution(Edge[] edges, int[][][] p, int[][] d, int l, int r) {
		if (l + 1 == r) {
			return;
		}
		int m = (l + r) >> 1;
		onlineConvolution(edges, p, d, l, m);
		for (Edge edge : edges) {
			int a = edge.a, b = edge.b;
			int[] ra = Convolution.convolution(d[a], l, m, p[a][b], 0, r - l, MOD);
			IntStream.range(m, Math.min(r, ra.length + l))
					.forEach(i -> d[b][i] = Convolution.safeMod(d[b][i] + ra[i - l], MOD));
			int[] rb = Convolution.convolution(d[b], l, m, p[a][b], 0, r - l, MOD);
			IntStream.range(m, Math.min(r, rb.length + l))
					.forEach(i -> d[a][i] = Convolution.safeMod(d[a][i] + rb[i - l], MOD));
		}
		onlineConvolution(edges, p, d, m, r);
	}
