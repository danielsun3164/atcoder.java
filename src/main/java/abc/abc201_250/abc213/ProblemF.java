 ...
 * 解説通りに実装したソースコード
 *
 * https://atcoder.jp/contests/abc213/submissions/24898387 にも参考
 */
public class ProblemF {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			NavigableMap<Integer, Long> stack = new TreeMap<>();
			long[] answer = new long[n];
			Arrays.fill(answer, 0L);
			long total = 0L;
			for (int i = 0; i < n - 1; i++) {
				total = add(stack, lcp[i], total);
				answer[sa[i + 1]] += total;
			}
			stack.clear();
			total = 0L;
			...
			IntStream.range(0, n).forEach(i -> sb.append(answer[i] + n - i).append(System.lineSeparator()));
	...
	/**
	 * stackにxを追加し、xより大きい数字をすべて削除し、合計値totalを計算しなおす
	 *
	 * @param stack
	 * @param x
	 * @param total 合計値
	 * @return 計算しなおした合計値total
	 */
	private static long add(NavigableMap<Integer, Long> stack, int x, long total) {
		long count = 1;
		while (!stack.isEmpty()) {
			if (stack.lastKey() < x) {
				break;
			}
			Entry<Integer, Long> entry = stack.pollLastEntry();
