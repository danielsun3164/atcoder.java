 ...
 * 解説通りに実装したソースコード
 *
 * https://atcoder.jp/contests/abc217/editorial/2594 の解説の実装
 */
public class ProblemG別回答2 {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			long[] g = new long[n + 1], h = new long[n + 1];
			g[0] = 0L;
			IntStream.rangeClosed(1, n).forEach(k -> g[k] = powMod(npr(fact, invFact, k, q), m - r)
					* powMod(npr(fact, invFact, k, q + 1), r) % MOD * invFact[k] % MOD);
			IntStream.rangeClosed(0, n).forEach(i -> h[i] = (0 == (1 & i)) ? invFact[i] : -invFact[i]);
			long[] f = Convolution.convolution(g, h, (int) MOD);
			// TLE対応のため、出力はStringBuilderを使用
			StringBuilder sb = new StringBuilder();
			IntStream.rangeClosed(1, n).forEach(i -> sb.append(f[i]).append(System.lineSeparator()));
	 ...
	 * @param n
	 * @param r
	 * @return n P r mod MOD
	 */
	private static long npr(long[] fact, long[] invFact, int n, int r) {
		return ((n >= r) && (n > 0) && (r > 0)) ? fact[n] * invFact[n - r] % MOD : 0L;
	}
