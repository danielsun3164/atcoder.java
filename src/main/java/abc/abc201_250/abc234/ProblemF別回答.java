 ...
 * 解説通りに実装したソースコード
 *
 * https://kazun-kyopro.hatenablog.com/entry/ABC/234/F の実装<br/>
 * https://atcoder.jp/contests/abc234/submissions/28401513 にも参考
 */
public class ProblemF別回答 {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			IntStream.range(0, N).filter(i -> freq[i] > 0).forEach(i -> que.add(Arrays.copyOf(invFact, freq[i] + 1)));
			while (que.size() >= 2) {
				long[] a = que.poll(), b = que.poll();
				que.add(Convolution.convolution(a, b, MOD));
			}
			long[] a = que.poll();
			System.out.println(IntStream.range(1, a.length).mapToLong(i -> fact[i] * a[i] % MOD).sum() % MOD);
		}
	}
