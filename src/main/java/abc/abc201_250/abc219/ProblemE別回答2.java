...
/**
 * https://atcoder.jp/contests/abc219/submissions/26559717 を参考に作成
 */
public class ProblemE別回答2 {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			int end = N * N;
			int village = IntStream.range(0, end).reduce(0, (s, i) -> s | (scanner.nextInt() << i));
			System.out.println(IntStream.range(0, 1 << end).filter(bit -> 0 == (~bit & village)).filter(bit -> {
				DisjointSetUnion dsu = new DisjointSetUnion(end + 1);
				IntStream.range(0, N).forEach(i -> IntStream.range(0, N).forEach(j -> {
					if (0 == ((1 << (i * N + j) & bit))) {
						if ((0 == i) || (N - 1 == i) || (0 == j) || (N - 1 == j)) {
							dsu.merge(i * N + j, end);
						}
						if ((i < N - 1) && (0 == ((1 << ((i + 1) * N + j) & bit)))) {
							dsu.merge(i * N + j, (i + 1) * N + j);
						}
						if ((j < N - 1) && (0 == ((1 << (i * N + j + 1) & bit)))) {
