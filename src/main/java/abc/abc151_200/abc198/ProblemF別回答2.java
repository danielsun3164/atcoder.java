...
public class ProblemF別回答2 {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			// 母関数は https://oeis.org/A054473 に参照
			// 母関数の分子多項式 3*x^6+x^5+x^4+1
			long[] p = { 1, 0, 0, 0, 1, 1, 3 };
			// 母関数の分母多項式 (1-x^4)*(1-x^3)^2*(1-x^2)^2*(1-x) の展開
			long[] q = { 1, -1, -2, 0, 2, 4, -1, -3, -3, -1, 4, 2, 0, -2, -1, 1 };
			System.out.println(bostanMori(p, q, s - 6));
		}
	}
	...
	private static long bostanMori(long[] p, long[] q, long n) {
		while (n > 0) {
			...
			// p(x) * q(-x) を計算する
			long[] newP = Convolution.convolution(p, qRev, MOD);
			// q(x) * q(-x) を計算する
			long[] newQ = Convolution.convolution(q, qRev, MOD);
			// lamda式用変数
			long n2 = n;
		...
		return p[0] * Convolution.invMod(q[0], MOD) % MOD;
	}
