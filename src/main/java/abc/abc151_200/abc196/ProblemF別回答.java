...
public class ProblemF別回答 {
	...
	/** Convolution用 1_000_000_007の場合はWAになる */
	private static final int MOD = 998_244_353;

	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			char[] ss = scanner.next().toCharArray(), st = scanner.next().toCharArray();
			int n = ss.length, m = st.length;
			int[] s1 = new int[n], s2 = new int[n];
			IntStream.range(0, n).forEach(i -> {
				s1[i] = ss[i] - '0';
				s2[i] = 1 - s1[i];
			});
			int[] t1 = new int[m], t2 = new int[m];
			// tは順序を逆にする
			IntStream.range(0, m).forEach(i -> {
				t1[i] = st[m - 1 - i] - '0';
				t2[i] = 1 - t1[i];
			});
			int[] ns1 = Convolution.convolution(s1, t2, MOD), ns2 = Convolution.convolution(s2, t1, MOD);
			System.out.println(IntStream.range(m - 1, n).mapToLong(i -> (ns1[i] + ns2[i])).min().getAsLong());
		}
	}
