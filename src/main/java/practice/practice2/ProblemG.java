...
/**
 * https://atcoder.github.io/ac-library/master/document_ja/scc.html を参考に作成
 */
public class ProblemG {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			IntStream.range(0, m).forEach(i -> graph.addEdge(scanner.nextInt(), scanner.nextInt()));
			List<Integer>[] scc = graph.scc();
			System.out.println(scc.length);
			Arrays.stream(scc).forEach(list -> System.out
					.println(list.size() + " " + list.stream().map(String::valueOf).collect(Collectors.joining(" "))));
		}
	}
