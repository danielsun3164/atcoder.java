...
public class Problem065 {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			int r = scanner.nextInt(), g = scanner.nextInt(), b = scanner.nextInt(), k = scanner.nextInt();
			int x = scanner.nextInt(), y = scanner.nextInt(), z = scanner.nextInt();
			int rMin = k - y, rMax = r, gMin = k - z, gMax = g, bMin = k - x, bMax = b;
			if ((rMax < rMin) || (gMax < gMin) || (bMax < bMin)) {
				System.out.println(0);
				return;
			}
			int n = max(r, g, b, k, x, y, z);
			...
			long[] ar = new long[r + 1], ag = new long[g + 1], ab = new long[b + 1];
			IntStream.rangeClosed(0, r).forEach(i -> ar[i] = ncr(fact, invFact, r, i));
			IntStream.rangeClosed(0, g).forEach(i -> ag[i] = ncr(fact, invFact, g, i));
			IntStream.rangeClosed(0, b).forEach(i -> ab[i] = ncr(fact, invFact, b, i));
			long[] p1 = new long[r + 1], p2 = new long[g + 1];
			Arrays.fill(p1, 0L);
			Arrays.fill(p2, 0L);
			System.arraycopy(ar, rMin, p1, rMin, rMax + 1 - rMin);
			System.arraycopy(ag, gMin, p2, gMin, gMax + 1 - gMin);
			long[] p3 = Convolution.convolution(p1, p2, (int) MOD);
			System.out.println(IntStream.rangeClosed(bMin, bMax).filter(i -> (0 <= (k - i)) && (k - i <= p3.length))
					.mapToLong(i -> p3[k - i] * ab[i] % MOD).reduce(0L, (s, l) -> (s + l) % MOD));
		}
	}
