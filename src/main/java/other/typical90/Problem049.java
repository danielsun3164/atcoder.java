...
public class Problem049 {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			Path[] paths = IntStream.range(0, m)
					.mapToObj(i -> new Path(scanner.nextLong(), scanner.nextInt() - 1, scanner.nextInt()))
					.sorted((x, y) -> Long.compare(x.c, y.c)).toArray(Path[]::new);
			DisjointSetUnion dsu = new DisjointSetUnion(n + 1);
			long answer = Arrays.stream(paths).filter(path -> !dsu.same(path.l, path.r)).mapToLong(path -> {
				dsu.merge(path.l, path.r);
				return path.c;
			}).sum();
			System.out.println((1 == dsu.groupNum) ? answer : -1L);
		}
	}
