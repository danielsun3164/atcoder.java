...
/**
 * https://atcoder.jp/contests/abl/submissions/17016954 を参考に作成したソースコード
 */
public class ProblemF {
	...
	public static void main(String[] args) {
		try (Scanner scanner = new Scanner(System.in)) {
			...
			// fact[i]はiの階乗 mod MOD、invFact[i]はiの階乗^(-1) mod MOD
			long[] fact = new long[N], invFact = new long[N];
			fact[0] = 1L;
			IntStream.range(1, N).forEach(i -> fact[i] = fact[i - 1] * i % MOD);
			invFact[N - 1] = invMod(fact[N - 1]);
			IntStream.range(1, N).map(i -> N - 1 - i).forEach(i -> invFact[i] = invFact[i + 1] * (i + 1) % MOD);
			// semiFact[i]は(2i-1)の半階乗
			long[] semiFact = new long[N];
			semiFact[0] = 1L;
			IntStream.range(1, N).forEach(i -> semiFact[i] = semiFact[i - 1] * (2 * i - 1) % MOD);
			// 各数字の出現回数を保存するマップ
			...
			map.values().stream().forEach(m -> que.addLast(IntStream.range(0, m / 2 + 1).mapToLong(j -> {
				long tmp = fact[m] * invFact[2 * j] % MOD * invFact[m - 2 * j] % MOD * semiFact[j] % MOD;
				if (1 == (j & 1)) {
					tmp = (MOD - tmp) % MOD;
				}
				return tmp;
			...
			while (que.size() > 1) {
				que.addLast(Convolution.convolution(que.poll(), que.poll(), MOD));
			}
			long[] v = que.poll();
			System.out.println(LongStream.range(0, v.length).reduce(0L,
					(answer, i) -> (answer + v[(int) i] * semiFact[n - (int) i]) % MOD));
		}
	}

	private static long invMod(long x) {
		return Convolution.invMod(x, MOD);
	}
